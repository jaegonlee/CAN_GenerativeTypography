...
void manipulateMesh(HE_Mesh mesh) {
    ...
    HE_Selection selection = getRandomSelection(mesh, 0.05); 
    selection.subdivide(new HES_CatmullClark());
    selection = getRandomSelection(mesh, 0.10);
    selection.modify(new HEM_VertexExpand().setDistance(10));
    selection = getRandomSelection(mesh, 0.2);
    selection.modify(new HEM_Extrude().setDistance(15).setRelative(true).setChamfer(0.55));
    selection = getRandomSelection(mesh, 0.1);
    selection.modify(new HEM_Extrude().setDistance(10).setRelative(true).setChamfer(0.75));
    selection = getRandomSelection(mesh, 0.05);
    selection.subdivide(new HES_CatmullClark());
}
...
void colorFaces(HE_Mesh mesh) {
    ...
    for (HE_Face face : mesh.getFacesAsArray ()) {
        ...
        face.setUserLabel(color(map(c.xf() + c.yf(), -500, 500, 0.15, 0.65), 0.65, 1));
    }
...
PShape createPShapeFromHemesh(HE_Mesh mesh, boolean perVertexNormals) {
    ...
    for (int i=0; i<facesHemesh.length; i++) {
        ...
        shape.fill(faceArray[i].getUserLabel());
        for (int j = 0; j < 3; j++) {
